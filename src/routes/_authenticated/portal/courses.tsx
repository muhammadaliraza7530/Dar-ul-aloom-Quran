  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-muted-foreground">{ur ? "آپ کے فعال کورسز اور اساتذہ کی تفصیل" : "Your enrolled courses and teacher details"}</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-border bg-card p-12 text-center text-muted-foreground">
          ...
          <p className="text-sm">You will see your course details here once assigned by the administration.</p>
        </div>
      ) : (
            ...
            <div key={course.id} className="rounded-lg border border-border bg-card p-6 shadow-sm">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <span className={`mt-1 inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${course.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
              ...
              <dl className="mt-6 grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <dd className="mt-1 font-semibold text-foreground">{course.teacher_name || "Pending"}</dd>
                </div>
                <div>
                  <dt className="font-medium text-muted-foreground">Schedule</dt>
                  <dd className="mt-1 font-semibold text-foreground">{course.schedule || "To be decided"}</dd>
                </div>
                {course.started_on && (
                  <div className="col-span-2">
                    <dt className="font-medium text-muted-foreground">Started On</dt>
                    <dd className="mt-1 text-foreground">{new Date(course.started_on).toLocaleDateString()}</dd>
                  </div>
                )}
              </dl>
            </div>
