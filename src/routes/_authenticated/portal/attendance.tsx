  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-muted-foreground">{ur ? "حالیہ کلاسز کی حاضری کا ریکارڈ" : "Record of your recent classes"}</p>
      </div>
        ...
        <table className="min-w-full divide-y divide-border">
          ...
          <tbody className="divide-y divide-border bg-card">
            {isLoading ? (
              ...
              <tr><td colSpan={4} className="px-6 py-4 text-center text-sm text-muted-foreground">No attendance records found.</td></tr>
            ) : (
              attendance?.map((record) => (
                <tr key={record.id}>
                  <td className="whitespace-nowrap px-6 py-4 text-sm text-foreground">{new Date(record.class_date).toLocaleDateString()}</td>
                  <td className="whitespace-nowrap px-6 py-4 text-sm text-foreground">{(record.enrollments as any)?.course_name || "-"}</td>
                  <td className="whitespace-nowrap px-6 py-4 text-sm">
                    <span className={`inline-flex rounded-full px-2 text-xs font-semibold leading-5 ${record.status === 'present' ? 'bg-green-100 text-green-800' : record.status === 'absent' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}`}>
