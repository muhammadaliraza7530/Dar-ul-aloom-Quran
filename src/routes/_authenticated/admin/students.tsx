      ...
      <div className="overflow-x-auto rounded-lg border border-border bg-card shadow-sm">
        <table className="min-w-full divide-y divide-border">
          <thead className="bg-muted/50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium uppercase text-muted-foreground">ID</th>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium uppercase text-muted-foreground">City/Country</th>
          ...
          <tbody className="divide-y divide-border">
            {isLoading ? (
              ...
              <tr><td colSpan={6} className="px-6 py-4 text-center text-sm text-muted-foreground">No students found.</td></tr>
            ) : (
              students?.map((student) => (
                <tr key={student.id}>
                  <td className="px-6 py-4 text-sm font-semibold text-gold-dark">{student.student_id}</td>
                  ...
                  <td className="px-6 py-4 text-sm text-muted-foreground">{[student.city, student.country].filter(Boolean).join(", ")}</td>
