...
import { CheckCircle2, Clock, GraduationCap, MessageCircle, ShieldCheck, Star, UserRoundCheck } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({ meta: [
    { title: "Learn Quran Online | Dar ul Uloom Online Quran Academy & Islamic Center Quran Academy & Islamic Center" },
    ...
    { property: "og:title", content: "Learn Quran Online | Dar ul Uloom Online Quran Academy & Islamic Center Quran Academy & Islamic Center" },
  ...
  return <>
    <section className="relative isolate min-h-[720px] overflow-hidden bg-primary text-primary-foreground lg:min-h-[760px]">
      {heroImages.map((src, idx) => (
        <img key={src} src={src} alt="Quran teacher guiding a young student in an online lesson" className={`absolute inset-0 size-full object-cover object-[64%_center] transition-all duration-[4000ms] ease-out ${idx === currentImage ? "opacity-100 scale-105" : "opacity-0 scale-100"}`} fetchPriority={idx === 0 ? "high" : "auto"} />
      ))}
      ...
      <div className="relative mx-auto flex min-h-[720px] max-w-7xl items-center px-4 py-24 sm:px-6 lg:min-h-[760px]">
        <div className="max-w-2xl">
          ...
          <div className="mt-8 flex flex-wrap gap-3"><Button asChild variant="gold" size="lg"><a href="https://wa.me/923298503412?text=Assalamu%20Alaikum%2C%20I%20would%20like%20to%20book%20a%20free%20trial%20class." target="_blank" rel="noreferrer"><MessageCircle />{language === "ur" ? "مفت کلاس بک کریں" : "Book free trial"}</a></Button><Button asChild variant="heroOutline" size="lg"><a href="https://wa.me/923298503412" target="_blank" rel="noreferrer"><MessageCircle />{language === "ur" ? "واٹس ایپ کریں" : "WhatsApp us"}</a></Button></div>
