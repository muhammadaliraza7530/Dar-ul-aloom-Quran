...
function BlogPage() {
  ...
  return (
    <>
      <PageHero title="Blog & articles" urdu="مضامین" description="Practical guidance from our teachers on learning, memorizing and living the Quran." />
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-3">
          {blogPosts.map((post) => (
            <article key={post.slug} className="border border-border bg-card p-7 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-wider text-gold-dark">{post.date}</p>
              <h2 className="mt-3 font-display text-2xl text-primary">{language === "ur" ? post.urdu : post.title}</h2>
