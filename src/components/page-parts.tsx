...
export function SectionHeading({ eyebrow, title, urdu, centered = true }: { eyebrow: string; title: string; urdu?: string; centered?: boolean }) {
  const { language } = useLanguage();
  return <div className={centered ? "mx-auto mb-10 max-w-3xl text-center" : "mb-8 max-w-2xl"}><p className="mb-2 text-xs font-bold uppercase tracking-[0.2em] text-gold-dark">{eyebrow}</p><h2 className="font-display text-3xl text-primary sm:text-4xl">{language === "ur" && urdu ? urdu : title}</h2><div className={`mt-4 h-0.5 w-16 bg-gold ${centered ? "mx-auto" : ""}`} /></div>;
}
...
export function FeatureList({ items }: { items: string[] }) {
  return <ul className="grid gap-3 sm:grid-cols-2">{items.map((item) => <li key={item} className="flex items-start gap-3 text-sm leading-6"><span className="mt-0.5 grid size-5 shrink-0 place-items-center rounded-full bg-gold/20 text-gold-dark"><Check className="size-3" /></span>{item}</li>)}</ul>;
}
