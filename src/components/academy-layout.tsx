      ...
      <div className={language === "ur" ? "font-urdu" : "font-sans"}>
        <div className="bg-noir text-ivory">
          <div className="mx-auto grid min-h-10 max-w-7xl grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 text-xs sm:flex sm:justify-between sm:px-6">
            <div className="flex min-w-0 items-center gap-4">
              <a
                className="flex min-w-0 items-center gap-2 truncate hover:text-gold"
                href="tel:+923298503412"
              >
                <Phone className="size-3.5 shrink-0" />
                <span dir="ltr">+92 329 8503412</span>
              </a>
              <span className="hidden text-ivory/60 sm:inline">
                {language === "ur" ? "دنیا بھر میں آن لائن کلاسز" : "Online classes worldwide"}
              </span>
            </div>
            <div className="flex shrink-0 items-center border border-gold/40 p-0.5">
              <button
                onClick={() => setLanguage("en")}
                className={`min-w-10 px-2 py-1 ${language === "en" ? "bg-gold text-noir" : "text-ivory"}`}
              ...
              <button
                ...
                className={`min-w-10 px-2 py-1 font-urdu ${language === "ur" ? "bg-gold text-noir" : "text-ivory"}`}
        ...
        <header className="sticky top-0 z-50 border-b border-gold/25 bg-background/95 shadow-sm backdrop-blur">
          <div className="mx-auto grid max-w-7xl grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-4 py-3 sm:px-6 lg:grid-cols-[auto_minmax(0,1fr)_auto]">
            <Link to="/" className="flex min-w-0 items-center gap-3">
              <img
                src={logoUrl}
                alt="Dar ul Uloom Online Quran Academy & Islamic Center logo"
                className="size-16 shrink-0 rounded-full border-2 border-gold object-cover sm:size-20"
                width={80}
                height={80}
              />
              <div className="min-w-0">
                <p className="truncate font-display text-lg leading-tight text-primary sm:text-xl">
                  Dar ul Uloom Online Quran Academy & Islamic Center
                </p>
                <p className="font-urdu text-lg text-gold-dark sm:text-2xl leading-normal mt-1">
            ...
            >
              {nav.map(([en, ur, to]) => (
                <Link
                  ...
                  activeProps={{ className: "text-gold-dark border-b-2 border-gold" }}
                  className="px-2.5 py-3 text-sm font-semibold text-foreground transition-colors hover:text-gold-dark"
                >
                  {language === "ur" ? ur : en}
                </Link>
              ))}
        ...
        <footer className="border-t-4 border-gold bg-noir text-ivory">
          <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 md:grid-cols-3">
            <div>
              <img
                src={logoUrl}
                alt="Academy emblem"
                className="mb-4 size-24 rounded-full border-2 border-gold"
                width={96}
                height={96}
                loading="lazy"
              />
              ...
              <p className="mt-2 max-w-sm text-sm leading-7 text-ivory/70">
                {language === "ur"
                  ? "بچوں اور بڑوں کے لیے قرآن، عربی اور اسلامی تعلیم۔"
                  : "Quran, Arabic and Islamic education for children and adults worldwide."}
              </p>
            </div>
            <div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                {nav.slice(1).map(([en, ur, to]) => (
                  <Link key={to} to={to} className="text-ivory/70 hover:text-gold">
                    {language === "ur" ? ur : en}
                  </Link>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="mt-4 space-y-3 text-sm text-ivory/70">
                <p dir="ltr">WhatsApp: 0329 8503412</p>
                <p dir="ltr">WhatsApp: 0335 0909536</p>
          ...
          <div className="border-t border-ivory/10 px-4 py-5 text-center text-xs text-ivory/50">
            © 2026 Dar ul Uloom Online Quran Academy & Islamic Center
          </div>
        </footer>
        <a
          ...
          className="fixed bottom-5 end-5 z-50 grid size-14 place-items-center rounded-full bg-whatsapp text-primary-foreground shadow-xl transition-transform hover:scale-105"
        >
