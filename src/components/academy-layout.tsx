      ...
      <div className={language === "ur" ? "font-urdu" : "font-sans"}>
        ...
        <header className="sticky top-0 z-50 border-b border-gold/25 bg-background/95 shadow-sm backdrop-blur">
          <div className="mx-auto grid max-w-7xl grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-4 py-3 sm:px-6 lg:grid-cols-[auto_minmax(0,1fr)_auto]">
            <Link to="/" className="flex min-w-0 items-center gap-3">
              <img src={logoUrl} alt="Dar-ul-Uloom Online Quran Academy logo" className="size-16 shrink-0 rounded-full border-2 border-gold object-cover sm:size-20" width={80} height={80} />
              <div className="min-w-0">
                <p className="truncate font-display text-lg leading-tight text-primary sm:text-xl">Dar-ul-Uloom</p>
                <p className="hidden text-[11px] uppercase text-muted-foreground sm:block">Online Quran Academy & Islamic Center</p>
                <p className="font-urdu text-base text-gold-dark sm:text-lg">دارالعلوم آن لائن قرآن اکیڈمی</p>
              </div>
            </Link>
            ...
            <Button asChild variant="gold" className="hidden lg:inline-flex"><Link to="/free-trial">{language === "ur" ? "مفت کلاس بک کریں" : "Book Free Trial"}</Link></Button>

            <Button variant="ghost" size="icon" className="lg:hidden" onClick={() => setOpen((value) => !value)} aria-label="Toggle menu">{open ? <X /> : <Menu />}</Button>
          </div>
          {open && <nav className="border-t border-border bg-background px-4 py-4 lg:hidden">{nav.map(([en, ur, to]) => <Link key={to} to={to} className="block border-b border-border py-3 font-semibold">{language === "ur" ? ur : en}</Link>)}<Button asChild variant="gold" className="mt-4 w-full"><Link to="/free-trial">{language === "ur" ? "مفت کلاس بک کریں" : "Book Free Trial"}</Link></Button></nav>}
        </header>
        ...
        <footer className="border-t-4 border-gold bg-noir text-ivory">
          <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 md:grid-cols-3">
            <div><img src={logoUrl} alt="Academy emblem" className="mb-4 size-24 rounded-full border-2 border-gold" width={96} height={96} loading="lazy" /><h2 className="font-display text-2xl text-gold">Dar-ul-Uloom</h2><p className="mt-2 max-w-sm text-sm leading-7 text-ivory/70">{language === "ur" ? "بچوں اور بڑوں کے لیے قرآن، عربی اور اسلامی تعلیم۔" : "Quran, Arabic and Islamic education for children and adults worldwide."}</p></div>
